"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.SerialsController = void 0;
const common_1 = require("@nestjs/common");
const serials_service_1 = require("./serials.service");
const create_serial_dto_1 = require("./dto/create-serial.dto");
const update_serial_dto_1 = require("./dto/update-serial.dto");
let SerialsController = class SerialsController {
    constructor(serialsService) {
        this.serialsService = serialsService;
    }
    create(createSerialDto) {
        return this.serialsService.create(createSerialDto);
    }
    findAll() {
        return this.serialsService.findAll();
    }
    findOne(id) {
        return this.serialsService.findOne(id);
    }
    update(id, updateSerialDto) {
        return this.serialsService.update(id, updateSerialDto);
    }
    remove(id) {
        return this.serialsService.remove(id);
    }
};
exports.SerialsController = SerialsController;
__decorate([
    (0, common_1.Post)(),
    __param(0, (0, common_1.Body)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [create_serial_dto_1.CreateSerialDto]),
    __metadata("design:returntype", void 0)
], SerialsController.prototype, "create", null);
__decorate([
    (0, common_1.Get)(),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", []),
    __metadata("design:returntype", void 0)
], SerialsController.prototype, "findAll", null);
__decorate([
    (0, common_1.Get)(':id'),
    __param(0, (0, common_1.Param)('id')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String]),
    __metadata("design:returntype", void 0)
], SerialsController.prototype, "findOne", null);
__decorate([
    (0, common_1.Patch)(':id'),
    __param(0, (0, common_1.Param)('id')),
    __param(1, (0, common_1.Body)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String, update_serial_dto_1.UpdateSerialDto]),
    __metadata("design:returntype", void 0)
], SerialsController.prototype, "update", null);
__decorate([
    (0, common_1.Delete)(':id'),
    __param(0, (0, common_1.Param)('id')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String]),
    __metadata("design:returntype", void 0)
], SerialsController.prototype, "remove", null);
exports.SerialsController = SerialsController = __decorate([
    (0, common_1.Controller)('serials'),
    __metadata("design:paramtypes", [serials_service_1.SerialsService])
], SerialsController);
//# sourceMappingURL=serials.controller.js.map